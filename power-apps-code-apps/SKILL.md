...
description: "Build, develop, and deploy Power Apps Code Apps — code-first web applications that run inside the Power Platform. Use this skill when working on Power Apps Code Apps projects: scaffolding new apps, writing React components that use the @microsoft/power-apps SDK, adding data sources and connectors (Dataverse, SQL, SharePoint, Office 365), using generated CRUD services and typed models, running npx power-apps commands (init, push, run, add-data-source, list-datasets, list-tables, list-connection-references, delete-data-source), configuring Vite builds with the power-apps-vite plugin, deploying to Power Platform environments or diagnosing missing functionality caused by absent data sources or connectors. Triggers on: power.config.json files, @microsoft/power-apps imports, npx power-apps commands, Power Apps Code Apps projects."
license: MIT
metadata:
  version: 1.1.0
---
...
**SDK v1.0.4+**: All CLI commands use the npm-based CLI (`npx power-apps`). The PAC CLI (`pac code`) is no longer required. See [references/cli-commands.md](references/cli-commands.md) for the full command reference.
...
- **< 1.0.4**: The PAC CLI (`pac code`) was used for some commands — fully replaced by `npx power-apps` in v1.0.4.
...
- **Do NOT guess API names** — discover them via `npx power-apps connection-list`
...
- **Use `npx power-apps` for all CLI commands** — as of SDK v1.0.4, the npm CLI fully replaces the PAC CLI for Code Apps. Authentication is handled automatically.
