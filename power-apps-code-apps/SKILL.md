...
description: "Build, develop, and deploy Power Apps Code Apps — code-first web applications that run inside the Power Platform. Use this skill when working on Power Apps Code Apps projects: scaffolding new apps, writing React components that use the @microsoft/power-apps SDK, adding data sources and connectors (Dataverse, SQL, SharePoint, Office 365), using generated CRUD services and typed models, running npx power-apps commands (npx power-apps init, npx power-apps push, npx power-apps run) or pac CLI commands for data source management (pac code add-data-source), configuring Vite builds with the power-apps-vite plugin, deploying to Power Platform environments or diagnosing missing functionality caused by absent data sources or connectors. Triggers on: power.config.json files, @microsoft/power-apps imports, npx power-apps commands, pac code commands, Power Apps Code Apps projects."
license: MIT
metadata:
  version: 1.1.0
---
...
**ALWAYS read this skill before recommending or running any `npx power-apps` or `pac code` CLI command.** Do not rely on other context sources (e.g., copilot-instructions.md) for CLI syntax — this skill is the authoritative reference.

**SDK v1.0.4+**: The npm-based CLI (`npx power-apps`) replaces `pac code` for project init, local dev, and deployment. Data source management still uses `pac code` commands. See [references/cli-commands.md](references/cli-commands.md) for the full command reference.
...
- **Use `npx power-apps` for init/run/push** — as of SDK v1.0.4, these replace `pac code init`, `pac code run`, and `pac code push`. The npm CLI handles authentication automatically.
